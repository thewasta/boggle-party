...
describe("board-utils", () => {
  ...
  describe("areCellsAdjacent", () => {
    it("returns true for horizontally adjacent cells", () => {
      expect(areCellsAdjacent({ row: 1, col: 1 }, { row: 1, col: 2 })).toBe(
        true,
      );
    });

    it("returns true for vertically adjacent cells", () => {
      expect(areCellsAdjacent({ row: 1, col: 1 }, { row: 2, col: 1 })).toBe(
        true,
      );
    });

    it("returns true for diagonally adjacent cells", () => {
      expect(areCellsAdjacent({ row: 1, col: 1 }, { row: 2, col: 2 })).toBe(
        true,
      );
    });

    it("returns false for non-adjacent cells", () => {
      expect(areCellsAdjacent({ row: 0, col: 0 }, { row: 2, col: 2 })).toBe(
        false,
      );
    });

    it("returns false for same cell", () => {
      expect(areCellsAdjacent({ row: 1, col: 1 }, { row: 1, col: 1 })).toBe(
        false,
      );
    });
  });
