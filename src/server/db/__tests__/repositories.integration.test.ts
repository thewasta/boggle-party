...
describe("Database Repositories Integration Tests", () => {
  ...
  describe("GamesRepository", () => {
    ...
    it("should update game status", async () => {
      ...
      const updated = await gamesRepository.updateStatus(game.id, "playing", {
        started_at: new Date(),
      });
  ...
  describe("WordsRepository", () => {
    ...
    it("should check if word exists in game (case insensitive)", async () => {
      ...
      const existsLower = await wordsRepository.wordExistsInGame(
        game.id,
        "gato",
      );
      const existsUpper = await wordsRepository.wordExistsInGame(
        game.id,
        "GATO",
      );
      const notExists = await wordsRepository.wordExistsInGame(
        game.id,
        "PERRO",
      );
