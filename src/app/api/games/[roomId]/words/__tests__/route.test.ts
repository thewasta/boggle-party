...
describe("POST /api/games/[roomCode]/words", () => {
  ...
  it("should accept valid word submission", async () => {
    const request = new NextRequest(
      `http://localhost:3000/api/games/${roomCode}/words`,
      {
        method: "POST",
        body: JSON.stringify({
          playerId,
    ...
    const response = await POST(request, {
      params: Promise.resolve({ roomId: roomCode }),
    });
  ...
  it("should return 404 for non-existent room", async () => {
    const request = new NextRequest(
      `http://localhost:3000/api/games/non-existent-room-id/words`,
      {
        method: "POST",
        body: JSON.stringify({
          playerId,
    ...
    const response = await POST(request, {
      params: Promise.resolve({ roomId: "non-existent-room-id" }),
    });
    const data = await response.json();
  ...
  it("should reject submission when game is not in progress", async () => {
    ...
    const request = new NextRequest(
      `http://localhost:3000/api/games/${roomWithoutGame}/words`,
      {
        method: "POST",
        body: JSON.stringify({
          playerId: host.id,
    ...
    const response = await POST(request, {
      params: Promise.resolve({ roomId: roomWithoutGame }),
    });
    const data = await response.json();
