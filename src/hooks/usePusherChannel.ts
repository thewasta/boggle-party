    ...
    try {
      ...
      if (handlers.onPlayerJoined) {
        channel.bind(PUSHER_EVENTS.PLAYER_JOINED, (data: unknown) => {
          handlersRef.current.onPlayerJoined?.(
            data as Parameters<NonNullable<typeof handlers.onPlayerJoined>>[0],
          );
        });
      }
      ...
      if (handlers.onPlayerLeft) {
        channel.bind(PUSHER_EVENTS.PLAYER_LEFT, (data: unknown) => {
          handlersRef.current.onPlayerLeft?.(
            data as Parameters<NonNullable<typeof handlers.onPlayerLeft>>[0],
          );
        });
      }
      ...
      if (handlers.onGameStarted) {
        channel.bind(PUSHER_EVENTS.GAME_STARTED, (data: unknown) => {
          handlersRef.current.onGameStarted?.(
            data as Parameters<NonNullable<typeof handlers.onGameStarted>>[0],
          );
        });
      }
      ...
      if (handlers.onGameEnded) {
        channel.bind(PUSHER_EVENTS.GAME_ENDED, (data: unknown) => {
          handlersRef.current.onGameEnded?.(
            data as Parameters<NonNullable<typeof handlers.onGameEnded>>[0],
          );
        });
      }
      ...
      if (handlers.onRoomClosed) {
        channel.bind(PUSHER_EVENTS.ROOM_CLOSED, (data: unknown) => {
          handlersRef.current.onRoomClosed?.(
            data as Parameters<NonNullable<typeof handlers.onRoomClosed>>[0],
          );
        });
      }
      ...
      if (handlers.onWordFound) {
        channel.bind(PUSHER_EVENTS.WORD_FOUND, (data: unknown) => {
          handlersRef.current.onWordFound?.(
            data as Parameters<NonNullable<typeof handlers.onWordFound>>[0],
          );
        });
      }
      ...
      if (handlers.onRevealWord) {
        channel.bind(PUSHER_EVENTS.REVEAL_WORD, (data: unknown) => {
          handlersRef.current.onRevealWord?.(
            data as Parameters<NonNullable<typeof handlers.onRevealWord>>[0],
          );
        });
      }
      ...
      if (handlers.onResultsComplete) {
        channel.bind(PUSHER_EVENTS.RESULTS_COMPLETE, (data: unknown) => {
          handlersRef.current.onResultsComplete?.(
            data as Parameters<
              NonNullable<typeof handlers.onResultsComplete>
            >[0],
          );
        });
      }
      ...
      if (handlers.onRematchRequested) {
        channel.bind(PUSHER_EVENTS.REMATCH_REQUESTED, (data: unknown) => {
          handlersRef.current.onRematchRequested?.(
            data as Parameters<
              NonNullable<typeof handlers.onRematchRequested>
            >[0],
          );
        });
      }
